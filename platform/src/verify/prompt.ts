...
const SYSTEM_PROMPT = [
  ...
  "",
  "Citing evidence correctly — every observation MUST follow these rules. They are generic and",
  "apply to any page:",
  "- Cite only evidence genuinely attributable to the supplied snapshotId and that EXACT ref.",
  "  Put the exact visible accessible name or value in observedText — never a summary, a",
  "  paraphrase, or a synthetic line you assembled yourself.",
  "- You may quote a full serialized node line (e.g. role \"name\" [ref=eN] ...) ONLY if you copy",
  "  it verbatim from the evidence for that same ref. Do not invent, complete, or alter one.",
  "- Visible text that sits directly inside a paragraph, status, or alert region as plain text",
  "  with no ref of its own is OWNED BY THAT CONTAINER: cite the container's ref. Do NOT cite a",
  "  nearby child, sibling, strong, link, heading, or textbox ref just because it is close to",
  "  the text.",
  "- For page metadata, cite the page/root ref: give the page title as its OWN observation, and",
  "  the URL as a SEPARATE observation when needed. Do NOT combine them into invented prose such",
  "  as `page \"...\", url ...` — only quote a line that actually exists in the evidence.",
  "- Never guess or substitute a nearby ref. If a fact is visible but no supplied ref can safely",
  "  own it, return INCONCLUSIVE (AMBIGUOUS_EVIDENCE) rather than manufacture a citation.",
  "- Each observation should carry the SMALLEST exact evidence fragment that supports the claim.",
  "",
  "Citation examples (neutral placeholders):",
  "- Anonymous text \"Saved.\" inside `paragraph [ref=e10]` → cite ref e10, observedText \"Saved.\"",
  "  (the paragraph owns the text). Citing a child `strong [ref=e11]` for \"Saved.\" is WRONG —",
  "  e11 owns only its own text.",
  "- Page title and URL → TWO observations at the page/root ref: one observedText \"<title>\", one",
  "  observedText \"<url>\". A single observedText like `page \"<title>\", url <url>` is WRONG — no",
  "  such node exists in the evidence.",
